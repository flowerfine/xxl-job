...
public class ActorSystemConfig {
    ...
    @Value("${spring.application.name:xxl-job-admin}")
    private String application;

    @Bean(destroyMethod = "terminate")
    public ActorSystem<SpawnProtocol.Command> actorSystem() {
        Config config = ConfigFactory.load("xxl-job.conf");
        ActorSystem<SpawnProtocol.Command> actorSystem = ActorSystem.create(Behaviors.setup(ctx -> SpawnProtocol.create()), application, config);
